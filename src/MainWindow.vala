namespace MyLovelyApp {
    public class MainWindow : Hdy.ApplicationWindow {
        public weak MyLovelyApp.Application app { get; construct; }

        // Widgets
        public MyLovelyApp.Widgets.HeaderBar toolbar;
        ...
        public MainWindow (MyLovelyApp.Application application) {
            Object (
                ...
                icon_name: "com.github.small_tech.my_lovely_app"
            );
        }
        ...
        private void create_layout () {
            ...
            toolbar = new MyLovelyApp.Widgets.HeaderBar ();
        ...
        private void restore_window_state () {
            ...
            MyLovelyApp.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = MyLovelyApp.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    MyLovelyApp.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                MyLovelyApp.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                MyLovelyApp.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                MyLovelyApp.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                MyLovelyApp.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            MyLovelyApp.saved_state.set ("window-position", "(ii)", x, y);
        }
